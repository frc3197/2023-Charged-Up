...
public class AlignConeNode extends CommandBase {
  ...
  int num;
//1.145
  double[] redCube = new double[]{14.7, 3.84};
  double[] blueCube = new double[]{1.21, 6.0};
  ...
  public AlignConeNode(DrivetrainSubsystem drive, PoseEstimatorSubsystem pose, Vision vision, int chosenIndex) {
    driveSub = drive;
    poseSub = pose;
    visionSub = vision;
  ...
  public void execute() {
    ...
    double xSpeed = -1*(poseSub.getCurrentPose().getX() - locations[index][0]);
